namespace CosmicLexicon.Foundation.GuardClauses
{
    ...
    public static class GuardClauseNumericExtensions
    {
        extension(IGuardClause guard)
        {
            ...
            public void OutOfRange<T>(T value, T minimum, T maximum, [CallerArgumentExpression("value")] string? parameterName = null)
            ...
            public void Zero<T>(T value, [CallerArgumentExpression("value")] string? parameterName = null)
                where T : INumber<T>
            {
                if (value == T.Zero)
                {
            ...
            public void Negative<T>(T value, [CallerArgumentExpression("value")] string? parameterName = null)
                where T : INumber<T>
            {
